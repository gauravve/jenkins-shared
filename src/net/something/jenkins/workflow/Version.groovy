...
class Version implements Serializable, Comparable {
  ...
  /**
   * Increment supplied version by semantic verb ['patch','minor','major']
   *
   * @param currentVersion
   * @param increment Semantic version verb
   * @return String
   */
  static String incrementVersion(String currentVersion, String semverVerb) {
    String nextVersion
    switch (semverVerb.toUpperCase()) {
      case ~/^MAJOR$/:
        nextVersion = new Version(currentVersion).incrementMajorVersion().toString()
        break
      case ~/^MINOR$/:
        nextVersion = new Version(currentVersion).incrementMinorVersion().toString()
        break
      case ~/^PATCH$/:
        nextVersion = new Version(currentVersion).incrementPatchVersion().toString()
        break
      default:
        assert false
        break
